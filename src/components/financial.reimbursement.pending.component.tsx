...
export class FinancialReimbursementPendingComponent extends React.Component<any, any> {
        ...
        <table className="table table-striped table-dark">
          ...
          {this.state.reimbursementList.map((item, i) => {
              ...
              return (
                  <tr key={i}>
                      <th scope="row">{i+1}</th>
                      <td> <a href="#/view/pending" onClick={this.setViewMode} id={item.username}>{item.username}</a></td>
  ...
  private getReimbursementsUser = (e) => {
    ...
    const url= `view/users/${e.target.id}/pending`;
    console.log(url);
  ...
  private getReimbursements = () => {
    ...
    NetService.getData('view/pending')
        .then((data) => {
            const newState = new Array();
            data.forEach((item) => {
                newState.push(item);
